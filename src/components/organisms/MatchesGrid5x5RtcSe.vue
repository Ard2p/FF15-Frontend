    ...
    <div class="grid-body top-grid">
      <template v-for="(round, ridx) in rounds">
        ...
        <div
          ...
          :key="'filler' + ridx"
          :class="`filler-column-x-x filler-round-${(ridx === (rounds.length - 1)) ? ridx - 2: ridx}`"
        >
          <div
          ...
          >
            <div :class="{['r' + ridx]: ridx === (rounds.length - 1)}"/><div/>
          </div>
        </div>
      </template>
...
export default {
  ...
  props: ['isShown', 'players', 'grids', 'ownTeam'],
